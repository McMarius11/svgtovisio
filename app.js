...
(function () {
    ...
    svgInput.addEventListener('input', () => {
        ...
        if (val && val.includes('<svg')) {
            ...
            convertBtn.disabled = !currentSvg;
        }
    });
    ...
    convertBtn.addEventListener('click', async () => {
        ...
        await convert(currentSvg);
    });

    function loadFile(file) {
        if (!file.name.endsWith('.svg') && file.type !== 'image/svg+xml') {
            log('Please select an SVG file.', 'error');
            return;
        }
    ...
    function showPreview(svgString) {
        ...
        // Ensure the embedded SVG scales properly
    ...
    async function convert(svgString) {
        ...
        try {
            // Parse SVG
            log('Parsing SVG...', 'info');
            const parser = new SvgParser(svgString);
